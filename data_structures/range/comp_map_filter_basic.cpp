...
TEST_CASE( "test object life time" )
{
    ...
    auto new_rng =
        items
        | ranges::views::transform( []( const auto &i ) { return Item{ i.value * 2 }; } )
        | ranges::views::filter( []( const auto &i ) { return i.value > 2; } );
    // range is lazy evaluated, meaning that at this stage, transform() and filter does
    // not take effect counter remains 6 (no new object creation/deletion activities)
    //
    ...
    // for_each() will consume this range, triggering the actual creation (of these 2
    // items), hence counter = 14
