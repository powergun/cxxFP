...
std::function< int( int ) > compose1( std::function< int( int ) > f,
...
TEST_CASE( "demo: compose by explicit typing" )
{
    CHECK_EQ(
        10,
        compose1( []( auto x ) { return x + 3; }, []( auto x ) { return x + 7; } )( 0 ) );
}
...
{
    // type check is still performed at compile time, try replace x + 3 with Item{} and
    // see the compiler error
    CHECK_EQ(
        10,
        compose2( []( auto x ) { return x + 3; }, []( auto x ) { return x + 7; } )( 0 ) );
}
...
{
    auto f1 = []( int n ) { return n + 1; } * []( int n ) { return n * 10; } *
              []( int n ) { return ( n > 10 ) ? 10 : n; };
    // f . g $ 5
    CHECK_EQ( f1( 15 ), 101 );

    auto f2 = compose( []( int n ) { return n + 1; },
                       []( int n ) { return n * 10; },
                       []( int n ) { return ( n > 10 ) ? 10 : n; } );
    CHECK_EQ( f2( 15 ), 101 );
}